...
int main(void)
{
    ...
    SetData(bt1, 1);  // bt에 1 저장
    SetData(bt2, 2);  // bt에 2 저장
    SetData(bt3, 3);  // bt에 3 저장
    SetData(bt4, 4);  // bt에 4 저장
    
    MakeLeftSubTree(bt1, bt2);  // bt2를 bt1의 왼쪽 자식 노드로
    MakeRightSubTree(bt1, bt3);  // bt3를 bt1의 오른쪽 자식 노드로
    MakeLeftSubTree(bt2, bt4);  // bt2를 bt1의 왼쪽 자식 노드로
