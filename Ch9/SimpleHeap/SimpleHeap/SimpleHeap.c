...
void HInsert(Heap * ph, HData data, Priority pr)
{
    ...
    while(idx != 1)
    {
        ...
        if(pr < (ph->heapArr[GetParentIDX(idx)].pr)) // 새 노드의 우선 순위 높다면
        {
            // 부모 노드를 한 레벨 내림, 실제로 내림
            ph->heapArr[idx] = ph->heapArr[GetParentIDX(idx)];
            
            // 세 노드를 한 레벨 올림, 실제로 올리지는 않고 인덱스 값만 갱신
...
HData HDelete(Heap * ph)
{
    ...
    // 아래의 변수 parentIdx에는 나지막 노드가 저장될 위치 정보가 담긴다.
    ...
    {
        if(lastElem.pr <= ph->heapArr[childIdx].pr) // 마지막 노드와 우선 순위 비교
            break; // 마지막 노드의 우선 순위가 높으면 반복문 탈출
